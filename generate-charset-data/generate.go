...
func main() {
	...
	if err := os.MkdirAll(pkg, 0777); err != nil {
		fatalf("cannot make package directory: %v", err)
	}
	for _, name := range names {
		writeFile(filepath.Join(pkg, name)+".go", tmpl, info{
			Path:    filepath.Join("datafiles", name),
			Package: pkg,
		})
	}
	writeFile(filepath.Join(pkg, "doc.go"), docTmpl, info{
		Package: pkg,
	})
}
